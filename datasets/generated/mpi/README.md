...
The generator's `config/specs/mpi.json` defines this dataset. Its `dataset_name` selects
...
|---|---|---|---|
| `canonical-4solute` | `canonical/canonical-4solute_synthetic.zip` | RA | Canonical 4-solute discrete distribution, RA with TI+RI noise (T1-01 shape, reference.json solute_count) — 1A: RA; s ≈ 2.5 + 5.0 + 9.0 + 14.0 S |
| `tb01-grid-node` | `boundary/tb01_grid_node/tb01-grid-node_synthetic.zip` | RA | Single solute exactly on a grid node, s=5S k=1.5 (TB-01) — 1A: RA; s ≈ 5.0 S |
| `tb02-near-degenerate` | `boundary/tb02_near_degenerate/tb02-near-degenerate_synthetic.zip` | RA | Two solutes 0.05S apart at equal loading (TB-02) — 1A: RA; s ≈ 5.0 + 5.0 S |
| `tb03-grid-edges` | `boundary/tb03_grid_edges/tb03-grid-edges_synthetic.zip` | RA | Solutes at s_min=1S and s_max=20S, nothing between (TB-03) — 1A: RA; s ≈ 1.0 + 20.0 S |
| `tb04a-od-low` | `boundary/tb04a_od_low/tb04a-od-low_synthetic.zip` | RA | Grid-node solute loaded to peak OD 1.40, the low end of the 1.4-1.6 band (TB-04a) — 1A: RA; s ≈ 5.0 S |
| `tb04b-od-high` | `boundary/tb04b_od_high/tb04b-od-high_synthetic.zip` | RA | Grid-node solute loaded to peak OD 1.60, the high end of the 1.4-1.6 band (TB-04b) — 1A: RA; s ≈ 5.0 S |
| `tb05-od-trace` | `boundary/tb05_od_trace/tb05-od-trace_synthetic.zip` | RA | Grid-node solute at trace loading, signal 0.02 (TB-05) — 1A: RA; s ≈ 5.0 S |
...
python3 scripts/test-corpus/generate_synthetic_data.py \
  --us-bin-dir /path/to/ultrascan/bin --output-root /path/to/test-data --spec mpi
```
